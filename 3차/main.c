...
void IJ2inst(unsigned int reg,Instruction *inst,int blocksize, int type, Hit *hit);// I,J- TYPE 혼용.
...
{
    FILE *fp, *fp2;
    fp = fopen(argv[4],"rb"); //argv[1] 에는 불러드릴 변수의 수.
    ...
    // }
    // else if (type == 2) // set index가 1이면 사실상 cache memory의 행 2번째~3번쨰를 의미함. 즉 set index가 n이면 cache memory에서 행 (2n~2n+1)
    // {
...
void IJ2inst(unsigned int reg, Instruction *inst,int blocksize,int type, Hit *hit)
{
    switch(reg/(1<<26)){
        case 8:
            strcpy(inst->op,"addi");
